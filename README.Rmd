...
Let's have a look at how far away the towers are from current crime, using the `nearest_` function, which treats the current locations as "facilities", and the individuals as "users". `facility_user_dist` takes the arguments `facility`, a data.frame with columns names "lat" and "long", and `user`, a data.frame with columns "lat" and "long". To reiterate, **facility** is where we want to build the surveillance towers, and **user** is each crime location. 
...
mc_20 <- max_coverage(facility = york_unselected,
                      user = dat_crime_not_cov,
                      n_added = 20,
                      distance_cutoff = 100)
)
