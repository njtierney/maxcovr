...
`maxcovr` aims to provide researchers and analysts with a fast, accurate and extendible implementation of Church's maximal covering location problem, while trying to adhere to [`tidyverse`](http://www.tidyverse.org) principles.  In the future `maxcovr` will be able to talk to commercial solvers like CPLEX and Gurobi, and will have a mechanism for extensions.
